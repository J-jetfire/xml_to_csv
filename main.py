...
def process_and_write_xml_chunk(xml_files, output_file, shared_fieldnames, lock):
            ...
            for item in root.findall('.//Item'):
                ...
                if product_name is not None and quantity is not None and price is not None and comment is not None:
                    ...
                    if len(local_rows) >= ROWS_LEN_TO_WRITE:
                        # Если достигнут размер чанка, записываем его в CSV с использованием мьютекса
                        with lock:
                            with open(output_file, 'a', newline='') as csv_file:
                                writer = csv.writer(csv_file)
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=pool_size) as executor:
        # Разбиваем файлы на чанки для обработки
        ...
        executor.map(process_and_write_xml_chunk, xml_files, [output_file] * len(xml_files),
